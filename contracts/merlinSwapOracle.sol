...
contract merlinSwapOracle {
    ...
    ) internal view returns (Observation memory observation) {
        (
    ...
    ) internal view returns (Observation memory oldestObservation) {
    ...
    function getState(address pool) internal view returns (State memory state) {
        (
            uint160 sqrtPrice_96,
            int24 currentPoint,
            uint16 observationCurrentIndex,
    ...
    function _getTWAPointFromTarget(
        address pool,
        uint32 targetTimestamp
    ) internal view returns (int24 point) {
    ...
    function getTWAPoint(
        address pool,
        uint256 delta
    ) external view returns (bool enough, int24 avgPoint, uint256 oldestTime) {
        State memory state = getState(pool);
        // get oldest observation
        Observation memory oldestObservation;
        oldestObservation = getOldestObservation(
            pool,
        ...
        if (
            uint256(oldestObservation.blockTimestamp) <= block.timestamp - delta
        ) {
    ...
    function sqrtPriceX96ToUint(
        ...
        return MulDivMath.mulDivFloor(numerator1, numerator2, 1 << 192);
    }
    ...
    function getSqrtPriceByDelta(
        ...
        if (delta == 0) {
            return LogPowMath.getSqrtPrice(state.currentPoint);
        }
        ...
        if (
            ...
            uint256 targetTime = block.timestamp - delta;
            int24 avgPoint = _getTWAPointFromTarget(pool, uint32(targetTime));
            return LogPowMath.getSqrtPrice(avgPoint);
        } else {
            uint256 targetTime = oldestObservation.blockTimestamp;
            int24 avgPoint = _getTWAPointFromTarget(pool, uint32(targetTime));
            return LogPowMath.getSqrtPrice(avgPoint);
